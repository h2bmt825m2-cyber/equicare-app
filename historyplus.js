...
(function(){
  ...
  const metrics={
    ...
    temp:{label:'Temperatur',icon:'🌡️',get:d=>d.temp!==''&&Number.isFinite(Number(d.temp))?Number(d.temp):null,format:v=>`${Number(v).toFixed(1).replace('.0','')} °C`},
    hum:{label:'Luftfeuchte',icon:'💧',get:d=>d.hum!==''&&Number.isFinite(Number(d.hum))?Number(d.hum):null,format:v=>`${Math.round(v)} %`,floor:0,ceil:100},
    ...
    uv:{label:'UV-Index',icon:'☀️',get:d=>Number.isFinite(Number(d.uv))?Number(d.uv):null,format:v=>Number(v).toFixed(1).replace('.0',''),floor:0},
    wind:{label:'Wind',icon:'💨',get:d=>Number.isFinite(Number(d.wind))?Number(d.wind):null,format:v=>`${Number(v).toFixed(1).replace('.0','')} km/h`,floor:0},
    rain:{label:'Regen',icon:'🌧️',get:d=>Number.isFinite(Number(d.precipitation))?Number(d.precipitation):null,format:v=>`${Number(v).toFixed(1).replace('.0','')} mm`,floor:0},
    pollen:{label:'Pollen',icon:'🌾',get:d=>Number.isFinite(Number(d.pollenMax))?Number(d.pollenMax):null,format:v=>`${Number(v).toFixed(1).replace('.0','')} /m³`,floor:0},
    insectWeather:{label:'Insekten-Wetter',icon:'🦟',get:d=>Number.isFinite(Number(d.insectWeatherScore))?Number(d.insectWeatherScore):null,format:v=>weatherInsectLabel[Math.max(0,Math.min(4,Math.round(v)))]||'—',floor:0,ceil:4}
  };
